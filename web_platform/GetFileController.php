...
class GetFileControllerCore extends FrontController
{
    ...
    public function init()
    {
        if (isset($this->context->employee) && $this->context->employee->isLoggedBack() && Tools::getValue('file')) {
            ...
            $info1 = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql1);
            
            $orderReference = $info1['reference'];
            $idCostumer = $info1['id_customer'];
        
            //Second insert data to ps_webservice
            DB::getInstance()->insert('webservice', array(
                'reference' => pSQL(strval($orderReference)),
                'id_customer' => (int)$idCostumer,
                'key' => pSQL(strval($key)),
                'download_date' => null,
            ));
